...
namespace ArcherCore.Extensions
{
    public static class ConversionExtensions
    {
        ...
        {
            if (Validation.ValidState.IsNull(o))
                return defVal;
            ...
            if (defVal)
            {
                if (Validation.ValidState.IsFalse(s))
                    return false;
            }
            else
            {
                if (Validation.ValidState.IsTrue(s))
                    return true;
            }
        ...
        public static int ToIntSloppy(string? s, int offset = 0)
        {
            ...
            for (int i = offset; i < s.Length; i++)
            {
                ...
                if (leadSpace && char.IsWhiteSpace(ch))
                    continue;
                if (!StringUtilities.IsDigit1(ch))
                    break;
        ...
        public static int? ToId(int? id)
        {
            ...
            if (!Validation.ValidState.IsValidId(id))      // kInvalidId
                return null;
            return id;
        }
        public static int? ToId(Enum id)
        {
            // We use 0 as null sometimes from the db . convert it back to null for storage in the db.
            int idN = id.ToInt();
            if (!Validation.ValidState.IsValidId(idN))      // kInvalidId
                return null;
            return idN;
        }
        ...
        {
            // may be cast to enum ?
            return id ?? Validation.ValidState.kInvalidId;
        }

        public static string? ToUnique(string? id)
        {
            // A string FK to some other table.
            if (!Validation.ValidState.IsValidUnique(id))
        ...
        public static object? ChangeType(object? value, Type convertToType)
        {
            ...
            Validation.ValidState.ThrowIfNull(convertToType, nameof(convertToType));
            if (Validation.ValidState.IsNull(value))  // NULL or DBNull is always null.
                return null;
            ...
            {
                return Enum.ToObject(convertToType, ToInt(value));
            }
        ...
        public static List<ItemType> ToListDupe<ItemType>(System.Collections.Generic.IEnumerable<ItemType> a)
        {
